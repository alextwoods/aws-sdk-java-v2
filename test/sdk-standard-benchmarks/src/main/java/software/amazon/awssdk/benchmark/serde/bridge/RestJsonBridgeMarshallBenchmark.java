...
/**
 * Bridge counterpart to {@link software.amazon.awssdk.benchmark.serde.RestJsonMarshallBenchmark}.
 *
 ...
 *
 * <ol>
 *   <li>map the v2 {@code SdkPojo} to a smithy-java {@link Document}
 *       ({@link SdkPojoToDocument}, generic walk of {@code sdkFields()}), then</li>
 *   <li>run smithy-java's schema-based serialization:
 *       {@code RestJsonClientProtocol#createRequest(operation, input, context, endpoint)}.</li>
 * </ol>
 *
...
@Fork(3)
public class RestJsonBridgeMarshallBenchmark {
    ...
    // Pre-built inputs so the @Benchmark methods time only createRequest (like the native
    // smithy-java serde benchmark, which pre-builds its input in @Setup).
    private BridgeStruct bridgeInput;
    private SerializableStruct generatedStyleInput;
    private SwitchBridgeStruct switchInput;

    @Setup
    @SuppressWarnings("unchecked")
    public void setup() throws Exception {
        // --- v2 input (identical to RestJsonMarshallBenchmark) ---
        ...
        Model smithyModel;
        try (InputStream is = RestJsonBridgeMarshallBenchmark.class.getResourceAsStream(SMITHY_MODEL_PATH)) {
        ...
        // Pre-build the wrapped inputs once (like native smithy-java's benchmark does).
        this.bridgeInput = BridgeStruct.of(operation.inputSchema(), request);
        this.generatedStyleInput = new GeneratedStylePutObjectStruct(operation.inputSchema(), request);
        this.switchInput = SwitchBridgeStruct.of(operation.inputSchema(), request);
    }
    ...
    /** The ceiling: what v2 codegen WOULD emit — a monomorphic, straight-line SerializableStruct. */
    @Benchmark
    public void generatedStyleMarshall(Blackhole bh) {
        bh.consume(protocol.createRequest(operation, generatedStyleInput, context, ENDPOINT));
    }

    /** Cache-only attempt to approach generated speed: int type-code switch instead of per-member lambdas. */
    @Benchmark
