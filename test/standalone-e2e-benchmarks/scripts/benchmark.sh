...
if [[ -n "$JAR" ]]; then
    ...
    if [[ ! -f "$DIR/target/classpath.txt" || ! -d "$DIR/target/classes" ]]; then
        ...
        # The repo's Maven wrapper, not `mvn`: this module's plugins need Maven 3.2.5+ and the
        # system mvn on the benchmark host is older than that.
        (cd "$DIR" && "$DIR/../../mvnw" -q package -DskipTests)
    fi
