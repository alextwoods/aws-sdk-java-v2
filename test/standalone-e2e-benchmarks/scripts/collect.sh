...
#                     ceiling on quiesce-mode warmup wall time (runner default: 60). Raise it when
#                     runs come back steady_state=false: cheap operations need more ops, not more
#                     time, and 60s can cut warmup off before JIT settles.
#   --reps N          clean timing repetitions per case (default: 3)
#   --timing-only     run only the timing phase. The cpu/alloc phases need async-profiler, which is
#                     not installed everywhere; without it they fail and only clutter the manifest.
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --warmup-max-seconds) WARMUP_MAX_SECONDS="$2"; shift 2 ;;
        --reps)        REPS="$2"; shift 2 ;;
...
TUNING_ARGS=()
# benchmark.sh forwards arguments it does not recognize to the runner, which is how the runner-side
# warmup ceiling gets through.
[[ -n "$WARMUP_MAX_SECONDS" ]] && TUNING_ARGS+=(--warmup-max-seconds "$WARMUP_MAX_SECONDS")
...
if [[ -n "$JAR" ]]; then
    ...
    # The repo's Maven wrapper, not `mvn`: this module's plugins need Maven 3.2.5+ and the system
    # mvn on the benchmark host is older than that.
    (cd "$DIR" && "$REPO/mvnw" -q package -DskipTests)
...
EXTRA_PHASES=3
[[ $TIMING_ONLY -eq 1 ]] && EXTRA_PHASES=0
TOTAL_RUNS=$(( ${#CLIENT_ARR[@]} * ${#SCENARIO_ARR[@]} * (REPS + EXTRA_PHASES) ))
...
- warmup: $WARMUP
- warmup ceiling: ${WARMUP_MAX_SECONDS:-runner default (60s)}
- phases: $([[ $TIMING_ONLY -eq 1 ]] && echo "timing only (--timing-only)" || echo "timing, cpu profile, alloc profile, metrics")
...
if [[ $TIMING_ONLY -eq 1 ]]; then
    {
        ...
        CSV_ROWS=0
        [[ -f "$RESULTS" ]] && CSV_ROWS=$(($(grep -c . "$RESULTS") - 1))
        echo "- results: \`results.csv\` ($CSV_ROWS data rows)"
        echo "- cpu/alloc/metrics phases skipped (--timing-only)"
    } >> "$MANIFEST"
    echo ""
    echo "Collection complete (timing only): $RUNDIR (runs: $RUN_NO, failures: $FAILURES)"
    [[ $FAILURES -eq 0 ]]
    exit $?
fi
