...
if [[ $SKIP_SDK_BUILD -eq 0 ]]; then
    ...
    #
    # codegen-maven-plugin is in the set because this branch changes codegen: the plugin has to be
    # reinstalled before services/dynamodb regenerates against it, or the jar ends up with a client
    # generated by a stale plugin.
    #
...
MVN_ARGS=(-q clean package -DskipTests -Dbenchmark.phase="$PHASE" -Dbenchmark.sdk.commit="$SDK_COMMIT")
if [[ -n "$SDK_VERSION" ]]; then
    MVN_ARGS+=(-Daws.sdk.v2.version="$SDK_VERSION")
fi
(cd "$DIR" && "$REPO/mvnw" "${MVN_ARGS[@]}")

BUILT="$DIR/target/bridge-$PHASE.jar"
if [[ ! -f "$BUILT" ]]; then
...
mkdir -p "$ARCHIVE"
ARCHIVED="$ARCHIVE/bridge-$PHASE-$SDK_COMMIT$SUFFIX.jar"
cp "$BUILT" "$ARCHIVED"
