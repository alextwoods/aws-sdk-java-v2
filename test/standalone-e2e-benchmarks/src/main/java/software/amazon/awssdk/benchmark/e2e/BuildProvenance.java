...
public final class BuildProvenance {
    ...
    /**
     * Abbreviates a commit SHA the way git does, and leaves anything that is not one alone.
     *
     * <p>The value can be a stand-in rather than a SHA — {@code published-2.46.10} for a jar built
     * against a Maven Central release, or {@code unrecorded}. Truncating those to 11 characters
     * produced {@code published-2} in a results file, which reads like a version and is not one.
     */
    private static String abbreviate(String sha) {
        boolean isSha = sha.length() >= 11 && sha.chars().allMatch(c -> Character.digit(c, 16) >= 0);
        return isSha ? sha.substring(0, 11) : sha;
    }
