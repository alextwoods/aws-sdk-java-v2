...
/**
 * The third RFC bridge (after interceptors and — eventually — credentials): wraps an AWS SDK
 * for Java <b>v2</b> {@link SdkHttpClient} as a smithy-java {@link ClientTransport}.
 *
 * <p>This lets the smithy-java runtime send its requests over any of v2's battle-tested HTTP
 * clients (URLConnection, Apache, Apache5, Netty, CRT) unchanged. In the "v2 SDK becomes a
 * shell over smithy-java" end state, this is how a customer's configured `httpClient(...)`
 * keeps working: smithy-java drives the call; v2's transport puts bytes on the wire.
 *
 * <p>Scope: synchronous {@link SdkHttpClient} only (the demo's DynamoDB client is sync). An
 * async bridge over {@code SdkAsyncHttpClient} is the analogous follow-up. Bodies are
 * <b>streamed</b>, not buffered: the request body is exposed to v2 as a
 * {@link ContentStreamProvider} that re-opens the smithy {@link DataStream} on demand (so
 * v2 retries get a fresh stream), and the response body is the v2 stream handed straight to
 * {@link DataStream#ofInputStream}. No {@code readAllBytes()} on the hot path.
 */
