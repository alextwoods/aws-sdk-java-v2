...
execution attributes from a per-client template, H11 builds the stage graph once per client configuration
with the response handler on the RequestExecutionContext): the per-call endpoint params
