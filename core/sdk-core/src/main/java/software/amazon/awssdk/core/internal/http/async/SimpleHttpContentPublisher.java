...
public final class SimpleHttpContentPublisher implements SdkHttpContentPublisher {
    ...
    /**
     * Ceiling on how much is allocated up front from the {@code Content-Length} header on the fallback path. The
     * header is normally exact, but it can be set by a request interceptor, and trusting an absurd value would turn a
     * request that merely fails into an {@link OutOfMemoryError}. Bodies larger than this still read correctly, just
     * via the growing path.
     */
    ...
    /**
     * The full request content. Only ever read, never written, so it is safe to hand out {@link ByteBuffer#duplicate()}
     * views of it to subscribers.
     */
    ...
    SimpleHttpContentPublisher(SdkHttpFullRequest request, int maxPresizedAllocation) {
        this.content = resolveContent(request, maxPresizedAllocation);
    }
    ...
    private static ByteBuffer resolveContent(SdkHttpFullRequest request, int maxPresizedAllocation) {
        ContentStreamProvider provider = request.contentStreamProvider().orElse(null);
        if (provider == null) {
            return EMPTY;
        }

        // Fast path: the content is already buffered in memory (the common case for non-streaming operations, whose
        // bodies were marshalled into a byte array). Avoids re-reading and re-buffering the whole body, which is
        // otherwise paid on every attempt.
        ByteBuffer buffered = provider.contentAsByteBufferOrNull();
        if (buffered != null) {
            return buffered;
        }

        return ByteBuffer.wrap(invokeSafely(() -> readFully(provider.newStream(),
                                                            presizeHint(request, maxPresizedAllocation))));
    }

    /**
     * Reads {@code stream} into an exactly-sized array.
     *
     * <p>When the length is known up front this allocates the result array once and reads straight into it. That
     * avoids the staging buffer, the doubling reallocations and the final defensive copy that
    ...
    private static byte[] readFully(InputStream stream, int presizeHint) throws IOException {
        ...
        // Either the stream is longer than advertised, or it is longer than the pre-size cap. Read the remainder
    ...
    /**
     * The marshallers and the body-rewriting pipeline stages set {@code Content-Length} for in-memory bodies, so this
     * is normally present and exact.
     *
        ...
        public void request(long n) {
            if (running) {
                ...
                if (n <= 0) {
                    ...
                    // duplicate() so that each subscriber (and each retry attempt) gets independent position/limit
                    // over the shared, read-only content.
                    s.onNext(content.duplicate());
                    s.onComplete();
                }
            }
        }
