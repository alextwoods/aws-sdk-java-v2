...
class SimpleHttpContentPublisherTest {
    ...
    /**
     * The zero-copy fast path: when the provider exposes its buffer, the publisher must use it without opening the
     * stream at all, and the published bytes must be the provider's bytes.
     */
    @Test
    void subscribe_whenProviderExposesBuffer_publishesWithoutReadingStream() {
        ...
        SdkHttpFullRequest request = requestBuilder().contentStreamProvider(provider)
    ...
    /**
     * The buffer handed out by the provider may share its backing store, so the publisher must not consume the
     * provider's view: two publishers over the same provider must both see the full content.
     */
    @Test
    void subscribe_whenProviderExposesBuffer_twoPublishersBothSeeFullBody() {
        ContentStreamProvider provider = ContentStreamProvider.fromByteArrayUnsafe(BODY);
        SdkHttpFullRequest request = requestBuilder().contentStreamProvider(provider)
                                                    .putHeader(Header.CONTENT_LENGTH, Integer.toString(BODY.length))
                                                    .build();
    ...
    /**
     * The ceiling exists so a bogus Content-Length cannot drive a huge allocation. The body still has to be published
     * in full, and the reported length has to come from the body rather than the header.
     */
    @Test
    ...
    /**
     * The published buffer has to line up with what {@link SimpleHttpContentPublisher#contentLength()} advertises,
     * since the HTTP clients use the two together to frame the request.
     */
    @Test
